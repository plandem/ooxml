...
func TestVML_replace(t *testing.T) {
	...
	//replace attribute with custom object
	...
	encoded2, err := xml.Marshal(&entity2)
	require.Nil(t, err)

	entity3 := &vml.Excel{}
	decoder = xml.NewDecoder(bytes.NewReader([]byte(encoded2)))
	err = decoder.DecodeElement(entity3, nil)
	require.Nil(t, err)

	require.Equal(t, entity2, entity3)
}